...
class BulkController
{
    ...
    {
        $langCode = $request["params"];
        $tableName = "product_" . $langCode;
        try {
            $indexStatus = $this->elasticsearch->createIndexIfNotExists("makinecim_" . $langCode);
        } catch (\Throwable $th) {
            echo "bu index zaten mevcut";
            die();
        }
        for ($i = 0; $i <= 252000; $i += 1000) {
            ...
            foreach ($products as $product) {
                $productArray = array();
                $productLang = getDataById($tableName, "product_id", $product["product_id"]);
                ...
                $productArray["categories"] = getCategoryHierarchy(intval($product["category_id"]), $langCode);
                ...
                $productArray["warehouse"] = $product["warehouse_content_" . $langCode];
                array_push($productsArray, $productArray);
            }
            $response  = $this->indexer->bulkCreateProducts($productsArray, $langCode);
